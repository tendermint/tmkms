...
use crate::{account, vote, PublicKey, merkle};
#[cfg(feature = "serde")]
...
impl Set {
    ...
    pub fn hash(self) -> merkle::Hash {
        ...
        let validator_bytes: Vec<Vec<u8>> = self.validators.into_iter().map(|x| x.hash_bytes()).collect();
        let validator_byteslices: Vec<&[u8]> = (&validator_bytes).into_iter().map(|x| x.as_slice()).collect();
...
mod tests {
    ...
    use signatory::{PublicKeyed, ed25519};
    use signatory_dalek;
